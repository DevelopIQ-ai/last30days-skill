...
**GetXAPI (DevelopIQ fork).** Set `GETXAPI_KEY` in the environment or
`~/.config/last30days/.env`. Set `LAST30DAYS_X_BACKEND=getxapi` to select it
exclusively. Without a pin it is the last automatic fallback on ordinary hosts;
...
| `LAST30DAYS_X_BACKEND` | Pins the X backend (`bird` / `xai` / `xurl` / `xquik` / `getxapi` / `grok` / `xapi`); doctor renders the pin and predicts "will use" accordingly. The unpinned auto chain is bird → xai → xurl → xquik → getxapi (grok and xapi are opt-in only). Pin `grok` to enable it; a leftover `~/.grok/auth.json` is never auto-selected. Pin `xapi` to use `X_BEARER_TOKEN` on an ordinary host. On a Grok Bot host the unpinned chain is the official chain (xapi → xai → xurl) and this pin is the only way to select a backend outside it; the pin keeps its exclusive, no-failover meaning there, and doctor names the pinned backend. |
